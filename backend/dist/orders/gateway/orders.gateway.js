...
let OrdersGateway = class OrdersGateway {
    emitStatus(order) { this.server.emit(`order:${order.id}`, { id: order.id, code: order.code, status: order.status, updatedAt: order.updatedAt }); this.server.emit(`restaurant:${order.restaurantId}`, { type: 'ORDER_STATUS_CHANGED', order }); }
};
